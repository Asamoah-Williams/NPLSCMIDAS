...
from npl_src.report_paths import repro_manifest
from npl_src.kpi_reporter import (
    MetricThresholds, RunStamp,
    rmse_pp, mae_pp, smape, mase, r2_log,
    log_monitoring, log_gates
)
from npl_src.drift import drift_report
from npl_src.notifier import send_alert
from pathlib import Path
import pandas as pd
import numpy as np
from npl_src.report_paths import ReportRoot  # use your existing path helper

from npl_src.db import DatabaseConnection  # use your existing DB helper
...
def _upsert_csv(csv_path: Path, df: pd.DataFrame, key_cols, date_cols=(), sort_by=(), round_cols=None) -> Path:
    ...
    if db_con.check_db_table_exists(csv_path):
        prev = db_con._read_sql(csv_path)
        for c in date_cols:
    ...
    db_con._append_sql(merged, csv_path)
    return csv_path


# --- ReportRoot directory resolvers (compatible with different implementations) ---
...
def save_forecast_path_authoritative(df_path: pd.DataFrame, reports_root: ReportRoot) -> dict:
    ...
    try:
        ...
        if not d.dropna(subset=["y_pred_pct"]).empty:
            ...
            plt.text(0.5, 0.5, "No forward values available for the latest run", ha="center", va="center",
                     transform=plt.gca().transAxes)
        plt.title("6-Month Forward Forecast Path")
        plt.xlabel("Target month");
        plt.ylabel("NPL forecast (%)")
...
def write_forecast_path_metrics_latest(df_actual_pred: pd.DataFrame, reports_root: ReportRoot,
                                       window_months: int = 12) -> str:
    data_dir, _ = _rr_dirs(reports_root)
    data_dir.mkdir(parents=True, exist_ok=True)
    ...
    for h in [1, 2, 3, 4, 5, 6]:
        ...
        lt = np.log(np.clip(yt / 100.0, 1e-6, None));
        lp = np.log(np.clip(yp / 100.0, 1e-6, None))
    ...
    out["window"] = f"last_{window_months}m_with_actuals"
...
def build_features_for_scoring(panel: pd.DataFrame, horizon: int, cfg: Dict | None = None) -> pd.DataFrame:
    ...
    F = importlib.import_module("npl_src.features")
...
def load_model_for_h(h: int, cfg: Dict):
    model_dir = MODELS_CAND # ROOT / cfg.get("models_folder", "data/interim/models")
    mtype = _model_type_for_h(h, cfg)

    if mtype == "midas_ml":
        p = model_dir / f"lgbm_midas_h{h}.pkl"
        if not p.exists(): raise FileNotFoundError(f"[midas_ml] expected model file missing: {p}")
        try:
            import joblib
        except Exception as e:
        ...
        try:
            setattr(m, "version_", cfg.get("model_version", "v0.0.0"))
        except Exception:
            pass
        return m
    ...
    if mtype == "sc_midas_ols":
        ...
        if not p.exists(): raise FileNotFoundError(f"[sc_midas_ols] expected model file missing: {p}")
        m = SCMIDASOLS.load(p)
        try:
            setattr(m, "version_", cfg.get("model_version", "v0.0.0"))
        except Exception:
...
# ---------- Actuals derivation (robust; includes your 'NPL' log column) -------
...
def build_backtest_heatmap_matrix(df_rec: pd.DataFrame, metric: str = "abs_error_pp") -> pd.DataFrame:
    ...
    if metric == "smape":
        ...
        sm = pd.Series(np.where(denom == 0, 0.0, 200.0 * (df["y_pred_pct"] - df["y_true_pct"]).abs() / denom),
                       index=df.index)
        df["metric_val"] = sm
    else:
        df["metric_val"] = (df["y_pred_pct"] - df["y_true_pct"]).abs()
    mat = (df.assign(target_month=pd.to_datetime(df["target_month"]))
           .pivot_table(index="target_month", columns="horizon", values="metric_val", aggfunc="mean"))
    mat = mat.sort_index();
...
def write_backtest_heatmap_reports(mat: pd.DataFrame, cfg: Dict, metric: str = "abs_error_pp") -> Dict[str, str]:
    ...
    if mat is None or mat.empty:
        db_con._append_sql(pd.DataFrame(), csv_path)
        # pd.DataFrame().to_csv(csv_path, index=False);
        return {"csv": str(csv_path), "png": ""}
    # mat.to_csv(csv_path, index=True)
    db_con._append_sql(mat, csv_path)
...
def _fi_from_model_and_X(model, X: pd.DataFrame, mtype: str) -> pd.DataFrame:
    ...
    if mtype == "sc_midas_ols":
        ...
        std = np.asarray(X.std(ddof=0), dtype=float);
        std[~np.isfinite(std)] = 0.0
        scaled = np.abs(coef) * np.where(std > 0, std, 1.0);
        s = scaled.sum()
...
def write_feature_importance_reports(h: int, df_fi: pd.DataFrame, cfg: Dict) -> Dict[str, str]:
    ...
    db_con._append_sql(df_sorted, csv_path)
    ...
    try:
        ...
        print(f"[monitoring] NOTE: could not render FI chart for h={h}: {e}");
        png_path = None
    return {"csv": str(csv_path), "png": str(png_path) if png_path else ""}
...
def write_actual_vs_pred_reports(df_rec: pd.DataFrame, cfg: Dict) -> Dict[str, str]:
    ...
    db_con._append_sql(df_rec[cols], csv_path)
    try:
        import matplotlib.pyplot as plt
        ...
        plt.title("Rolling RMSE (6m) by Horizon");
        plt.xlabel("Month");
        plt.ylabel("RMSE (pp)")
        plt.legend(ncol=3, fontsize=8);
        plt.grid(alpha=0.3);
        plt.tight_layout()
        plt.savefig(charts_dir / "actual_vs_pred_latest.png", dpi=160);
        plt.close()
        ...
        for h, g in df_rec.sort_values("target_month").groupby("horizon"):
            ...
            plt.grid(alpha=0.3);
            plt.tight_layout()
            plt.savefig(charts_dir / f"actual_vs_pred_h{h}_latest.png", dpi=160);
            plt.close()
    except Exception as e:
...
def build_model_comparison(df_rec: pd.DataFrame, cfg: Dict) -> pd.DataFrame:
    ...
    if not rows:
        return pd.DataFrame(columns=[
            "horizon", "model_type", "model_version", "rmse_pp", "mae_pp", "smape", "mase", "r2_log", "n_obs",
            "overall_rank"
        ])
    ...
    # Nice ordering
    cols = ["horizon", "model_type", "model_version", "overall_rank", "mase", "smape", "rmse_pp", "mae_pp", "r2_log",
            "n_obs"]
    return df[cols]
...
def write_model_comparison_reports(df_comp: pd.DataFrame, cfg: Dict) -> Dict[str, str]:
    ...
    db_con._append_sql(df_comp, csv_path)
    try:
        import matplotlib.pyplot as plt
        ...
        plt.tight_layout()
        plt.savefig(charts_dir / "model_comparison_MASE_latest.png", dpi=160);
        plt.close()
    except Exception as e:
    ...
    if not h_subset.empty and len(h_subset) != len(df_comp):
        ...
        db_con._append_sql(h_subset, data_dir / "model_comparison_h0_h6.csv")
        try:
            import matplotlib.pyplot as plt
            plt.figure(figsize=(8, 4))
            plt.bar(h_subset["horizon"].astype(int).astype(str), h_subset["mase"])
            plt.title("Model Comparison (H0..H6) — MASE");
            plt.xlabel("Horizon");
            plt.ylabel("MASE")
            plt.grid(axis="y", alpha=0.3);
            plt.tight_layout()
            plt.savefig(charts_dir / "model_comparison_h0_h6_MASE_latest.png", dpi=160);
            plt.close()
        except Exception as e:
...
def score_horizon(h: int, panel: pd.DataFrame, model, as_of: pd.Timestamp, cfg: Dict) -> pd.DataFrame:
    ...
    if mtype == "midas_ml":
        ...
        yhat_pp = _to_pp_from_log(model.predict(X))
...
def aggregate_kpis_monthly(df_h: pd.DataFrame) -> pd.DataFrame:
    def _one(g: pd.DataFrame):
        y_t = g["y_true_pct"].dropna();
        y_p = g["y_pred_pct"].dropna()
        if len(y_t) == 0 or len(y_p) == 0:
            return pd.Series(
                {"rmse_pp": np.nan, "mae_pp": np.nan, "smape": np.nan, "mase": np.nan, "r2_log": np.nan, "n_obs": 0})
...
def write_calibration_coeffs(reports_root, h: int, a: float, b: float, as_of: pd.Timestamp) -> str:
    ...
    _upsert_csv(csv_path, df, key_cols=["as_of_date", "horizon"], date_cols=["as_of_date"],
                sort_by=["as_of_date", "horizon"])
    ...
    def _rr_dirs(rr) -> Tuple[Path, Path]:
        ...
        if hasattr(rr, "data_dir"):
            ...
            try:
                ...
                charts_dir = Path(
                    charts_attr() if callable(charts_attr) else charts_attr) if charts_attr else _rr_base_dir(
                    rr) / "charts"
                return data_dir, charts_dir
            except Exception:
                pass
...
def main():
    ...
    for h in cfg["horizons"]:
        try:
            model = load_model_for_h(h, cfg)
        except FileNotFoundError as e:
            print(f"[monitoring] SKIP h={h}: {e}");
            skipped.append(h)
            continue
    ...
    obj = forecast()
    df_forecast = obj.run_main()
    df_forward_block = df_forecast[['run_ts_utc', 'anchor_date', 'horizon', 'forecast_date', 'y_pred_pp']].copy()
    df_forward_block['run_ts_utc'] = datetime.now(UTC).date()
    df_forward_block = df_forward_block.rename(columns={'run_ts_utc': 'as_of_date', 'anchor_date': 'info_month', 'horizon': 'horizon', 'forecast_date': 'target_month', 'y_pred_pp': 'y_pred_pct'})
    ...
    win_eval = int(cfg.get("monitoring", {}).get("gate_window", 24))
    ...
    # Commented to prevent displaying of forcast
    print(
        df_forward_block
        .assign(target_month=lambda d: d["target_month"])
    ...
    for h in sorted(set(df_preds["horizon"])):
        try:
            m = load_model_for_h(h, cfg)
            mtype = _model_type_for_h(h, cfg)
            X = build_features_for_scoring(panel, horizon=h, cfg=cfg)
            df_fi = _fi_from_model_and_X(m, X, mtype)
            df_fi["horizon"] = h
